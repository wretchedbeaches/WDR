module.exports =  {
  ...
  CalculateTopRanks: function(MAIN, pokemonID, formID, cap, lvl) {
      ...
      for(a = 0; a <= 15; a++) {
          for(d = 0; d <= 15; d++) {
              for(s = 0; s <= 15; s++) {
                  let currentStat = this.CalculateBestPvPStat(MAIN, pokemonID, formID, a, d, s, cap, lvl);
  ...
  CalculateBestPvPStat: function (MAIN, pokemonID, formID, attack, defense, stamina, cap, lvl) {
      let bestStat = 0;
      let level = 0;
  ...
  SearchTopRank: async function (MAIN, search, filter){
    // RUN CALCULATIONS
    let possible_cps = {};
    if(search.pokemon.pokemon_id == 413 || search.pokemon.pokemon_id == 416 || search.pokemon.pokemon_id == 478){
      possible_cps = await this.CalculatePossibleCPs(MAIN, search.pokemon.pokemon_id, search.pokemon.form, search.stats.atk, search.stats.def, search.stats.sta, 1, 'female', filter.league);
    }
    else {
      possible_cps = await this.CalculatePossibleCPs(MAIN, search.pokemon.pokemon_id, search.pokemon.form, search.stats.atk, search.stats.def, search.stats.sta, 1, 'male', filter.league);
    }
    ...
    for(var i = possible_cps.length - 1; i >= 0; i--){
      if(!unique_cps[possible_cps[i].pokemonID]){
        pvpRanks = this.CalculateTopRanks(MAIN, possible_cps[i].pokemon_id, possible_cps[i].form_id, filter.max_cp_range, filter.level);
