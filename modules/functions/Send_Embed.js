...
module.exports = (MAIN, type, raid_level, server, content, embed, channel_id) => {
  ...
  let bot = MAIN.BOTS[MAIN.Next_Bot].channels.cache.get(channel_id) ? MAIN.BOTS[MAIN.Next_Bot].channels.cache.get(channel_id) : MAIN.channels.cache.get(channel_id);
  if(!bot) { return console.error('Problem finding channel: '+channel_id+' using Bot: '+MAIN.Next_Bot); }
