...
module.exports.run = async (MAIN, target, quest, quest_reward, simple_reward, area, server, timezone, content, embed) => {
...
let pokestopbody = await MAIN.Get_ID(MAIN, quest.latitude, quest.longitude, pokestop.sprite, 'https://raw.githubusercontent.com/whitewillem/PMSF/develop/static/forts/Pstop.png');
  ...
  function send_embed(){
    ...
    // SEND EMBED TO CHANNEL
      ...
      return MAIN.Send_Embed(MAIN, 'quest', 0, server, content, quest_embed, target.id);
    }
  }
}
...
function Get_Quest_Task(MAIN, quest, server){
  switch(true){
	...
	let condition = MAIN.Get_Locale(MAIN, {pokemon_id: quest.conditions[0].info.pokemon_ids[0], form: 0}, server);
        return 'Catch '+quest.target+' '+condition.pokemon_name+'.';
      }
    ...
    case quest.template.indexOf('throw_great_hard') >= 0:
        return 'Make 5 Great Curveball Throws in a Row.'
    ...
    // USE GRB
    case quest.template.indexOf('use_berry') >= 0:
      return 'Use '+quest.target+' GOLDEN RAZZ BERRIES TO HELP CATCH POKÉMON.';
    ...
    case quest.template.indexOf('feed_treet') >= 0:
    ...
    // INCENSE
    case quest.template.indexOf('incense') >= 0:
      return 'Use an incense.'
