...
async function subscription_view(MAIN, message, nickname, prefix, discord){
  MAIN.pdb.query(`SELECT * FROM users WHERE user_id = ? AND discord_id = ?`, [message.author.id, discord.id], function (error, user, fields) {
    ...
    if(!user[0].quests){ return message.reply('You have no saved Quest subscriptions.').then(m => m.delete({ timeout: 5000, reason: 'It had to be done.' })).catch(console.error); }
    else{

      let user_quests = user[0].quests ? user[0].quests.split(',') : [];
  ...
  MAIN.pdb.query(`SELECT * FROM users WHERE user_id = ? AND discord_id = ?`, [message.author.id, discord.id], async function (error, user, fields) {
    ...
    switch (sub) {
      ...
      default:
        ...
        // CHECK IF THE USER ALREADY HAS SUBSCRIPTIONS AND ADD
        if(index >= 0){ return message.reply('You are already subscribed to this quest reward.').then(m => m.delete({ timeout: 10000, reason: 'It had to be done.' })).catch(console.error); }
  ...
  MAIN.pdb.query(`SELECT * FROM users WHERE user_id = ? AND discord_id = ?`, [message.author.id, discord.id], async function (error, user, fields) {
    ...
    if(!user[0].quests){
      ...
      switch (sub) {
        ...
        default:
        // DEFINED VARIABLES
        let quests = user[0].quests ? user[0].quests.split(',') : [];
        let index = quests.indexOf(sub);
        let rewards = MAIN.config.QUEST.Rewards.toString().toLowerCase().split(',');
        let reward_index = rewards.indexOf(sub.toLowerCase());
    ...
    switch(type){
      ...
      case 'Name':
        ...
        instruction = new MAIN.Discord.MessageEmbed()
          .setAuthor(nickname, message.author.displayAvatarURL)
          .setTitle('What Quest Reward would you like to Subscribe to?')
          .addField('Any Pokemon Name or one of the following:', reward_list, false)
        ...
        switch(true){
          ...
          case type.indexOf('Remove')>=0:
            ...
            for(let r = 0; r < MAIN.config.QUEST.Rewards.length+1; r++){
              if(r == MAIN.config.QUEST.Rewards.length+1){ message.reply('`'+message.content+'` doesn\'t appear to be a valid Quest reward. Please check the spelling and try again.').then(m => m.delete({ timeout: 5000, reason: 'It had to be done.' })).catch(console.error); break; }
          ...
          case type.indexOf('Time')>=0:
            if(message.content.length < 6 && message.content.indexOf(':') >= 0){
              ...
              if(parseInt(times[0]) >= 0 && parseInt(times[0]) < 23 && parseInt(times[1]) <= 59 && parseInt(times[1]) >= 0){
                ...
                message.reply('`'+message.content+'` doesn\'t appear to be a valid Time. Please check the requirements and try again.').then(m => m.delete({ timeout: 5000, reason: 'It had to be done.' })).catch(console.error); break;
              } break;
            } else{
              message.reply('`'+message.content+'` doesn\'t appear to be a valid Time. Please check the requirements and try again.').then(m => m.delete({ timeout: 5000, reason: 'It had to be done.' })).catch(console.error); break;
            } break;
