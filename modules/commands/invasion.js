...
module.exports.run = async (MAIN, message, prefix, discord) => {
  ...
  let stop_collection = new MAIN.Discord.Collection();
  let available_stops = [];
  ...
  do {
    ...
    if(sub.stop == 'cancel'){ return subscription_cancel(MAIN, nickname, message, prefix, available_stops, discord, stop_collection); }
      ...
      if(sub.stop == 'All'){ sub.stop = 'All'; got_name = true; }
        ...
        if(!matches[0]){
          message.reply('`'+sub.stop+'`, does not closely match any stop in the database.').then(m => m.delete({ timeout: 5000, reason: 'It had to be done.' })).catch(console.error);
        } else{
        ...
        switch(true){
          ...
          case type.indexOf('Type') >= 0:
            switch(message.content.toLowerCase()){
              ...
              case 'cliff': collector.stop('Executive Cliff'); break;
              case 'arlo': collector.stop('Executive Arlo'); break;
              case 'sierra': collector.stop('Executive Sierra'); break;
              case 'giovanni': collector.stop('Giovanni or Decoy'); break;
              default:
              let found = false;
              ...
              if(!found){ message.reply('`'+message.content+'` doesn\'t appear to be a valid Invasion Grunt Type. Please check the spelling and try again.').then(m => m.delete({ timeout: 5000, reason: 'It had to be done.' })).catch(console.error); }
            } break;
