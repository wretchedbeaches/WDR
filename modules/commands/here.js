...
module.exports.run = async (MAIN, message, raids, discord) => {
  ...
  function count_members(){
    ...
    [raids.gym_id], function (error, lobbys, fields) {
      ...
      let interest = `${member} is **at the raid**! There are:\`\`\`\n`
...
function update_status(){
  ...
  ['here', raids.gym_id, member.id], function (error, lobby, fields) {
    if(error){ console.error(error); }
  });
}
...
function insert_user(count){
  ...
  [raids.gym_id, member.id, count, 'here', count, 'here'], function (error, lobby, fields) {
    if(error){ console.error(error); }
  });
  count_members();
}
