...
async function pokemon_view(MAIN, message, member, pokemon, prefix, discord){
  ...
  let pokemon_color = typing.color, pokemon_type = typing.type;
  ...
  MAIN.rdmdb.query(`SELECT
    ...
    AND first_seen_timestamp <= ?`, [pokemon_id, start, end], function (error, stats, fields) {
      if(!stats || stats[0].total == 0 ){return message.reply('No Data Found').then(m => m.delete({ timeout: 5000, reason: 'It had to be done.' })).catch(console.error);}
      let genders = '';
      ...
      if(message.channel.type == 'dm'){
        ...
        return MAIN.Send_Embed(MAIN, 'chart', 0, discord, '', chart_embed, message.channel.id);
      }
      else {
