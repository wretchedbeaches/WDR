...
module.exports.run = async (MAIN, message, prefix, discord) => {
  ...
  do {
    ...
    if(gym.gym_name == 'cancel'){ return lobby_cancel(MAIN, member, message, prefix, available_gyms, discord, gym_collection); }
      ...
      if(!Array.isArray(gym.gym_name) && gym.gym_name.split(',')[0] == 'fuzzy'){
        ...
        if(!matches[0]){
          message.reply('`'+gym.gym_name+'`, does not closely match any gym in the database.').then(m => m.delete({ timeout: 5000, reason: 'It had to be done.' })).catch(console.error);
        } else{
  ...
  MAIN.pdb.query(`SELECT * FROM active_raids WHERE gym_id = ?`, [gym.gym_id], async function (error, active, fields) {
    ...
    if(active && active[0]){
      ...
      message.reply('Raid detected, creating Lobby...').then(m => m.delete({ timeout: 8000, reason: 'It had to be done.' })).catch(console.error);
    ...
    // NO RAID FOR THE SPECIFIED GYM
      ...
      message.reply('Creating Lobby..').then(m => m.delete({ timeout: 8000, reason: 'It had to be done.' })).catch(console.error);
          ...
          case type.indexOf('Time') >= 0:
            if(message.content.length < 6 && message.content.indexOf(':') >= 0){
              ...
              if(parseInt(times[0]) >= 0 && parseInt(times[0]) < 23 && parseInt(times[1]) <= 59 && parseInt(times[1]) >= 0){
                ...
                message.reply('`'+message.content+'` doesn\'t appear to be a valid Time. '+requirements).then(m => m.delete({ timeout: 5000, reason: 'It had to be done.' })).catch(console.error); break;
              } break;
            } else{
              message.reply('`'+message.content+'` doesn\'t appear to be a valid Time. '+requirements).then(m => m.delete({ timeout: 5000, reason: 'It had to be done.' })).catch(console.error); break;
            } break;
...
function lobby_cancel(MAIN, member, message, prefix, available_gyms, discord, gym_collection){
  let lobby_cancel = new MAIN.Discord.MessageEmbed().setColor('00ff00')
    .setAuthor(member.nickname, member.displayAvatarURL)
    .setTitle('Lobby Cancelled.')
    .setDescription('Nothing has been created.');
  message.channel.send(lobby_cancel).then(m => m.delete({ timeout: 5000, reason: 'It had to be done.' })).catch(console.error);
}
