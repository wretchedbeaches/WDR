...
module.exports.run = async (MAIN, lure, area, server, timezone) => {
  ...
  if(MAIN.debug.Subscriptions == 'ENABLED' && MAIN.debug.Lure == 'ENABLED'){ console.info('[SUBSCRIPTIONS] ['+MAIN.Bot_Time(null,'stamp')+'] [lure.js] Received '+MAIN.Get_Lure(MAIN, lure.lure_id)+' lure for '+server.name+'.'); }
    ...
    if(users && users[0]){
      users.forEach((user,index) => {
        ...
        if(user.lure && user.lure_status == 'ACTIVE'){
          ...
          let type = MAIN.Get_Lure(MAIN, lure.lure_id);
